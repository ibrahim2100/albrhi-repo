...
**YouTube Music 0.9.3 — تعليمات الذكاء الاصطناعي بعد تدقيق.** مزوّد Anthropic صار يطبّق مخطّط
JSON على مستوى الـAPI للترجمة وتوحيد العنوان واستخراج الكلمات، بعد أن كان يتجاهل طلب JSON الذي
يحترمه OpenAI وGemini؛ `max_tokens` اتّسع للتفكير على Opus 5 و5.5؛ والتعليمات بنبرة عادية دون حذف
أي متطلّب.

## v1.77.8

**يوتيوب 1.31.8 — «إزالته بعد الحفظ في الصور» صار يعمل مع النسخ التلقائي أيضاً.** كان يقرؤه
السحب اليدوي وحده، فمن شغّل المفتاحين بقي ملفّه في المركز بعد وصوله إلى الصور.

## v1.77.7

**يوتيوب 1.31.7 — رفض الحفظ في الصور يسمّي نفسه.** قياساً من `Info.plist` ليوتيوب 21.34.3:
التطبيق لا يعلن `NSPhotoLibraryAddUsageDescription`، فيسقط iOS على مفتاح القراءة ويظهر تنبيهٌ
نصّه عن رفع الوسائط لا عن حفظ تنزيل. وثلاثة أنواع رفض كانت جملة واحدة؛ صار كلٌّ يسمّي نفسه
لأن لها ثلاثة إصلاحات مختلفة.

## v1.77.6

**يوتيوب 1.31.6 — رسالتا رفض الحفظ في الصور تشيران إلى ما هو موجود فعلاً.** إحداهما كانت تعد
بتسليم الفيديو عبر ورقة مشاركة لا تُعرض في أيٍّ من مواضعها الثلاثة؛ صارتا تدلّان على إجراء
«مشاركة» القائم في نفس القائمة، وعلى مكان منح الإذن في إعدادات iOS.

## v1.77.5

**يوتيوب 1.31.5 — «لا تُحفظ في الاستوديو» تقول الآن لماذا.** الحفظ في الصور مُطفأ افتراضياً
بقرار مقصود والملف يبقى في مركز التنزيلات؛ أمّا لو كان المفتاح مُشغَّلاً ورُفض النسخ فكان
الرفض يُكتب في السجل وحده، والشاشة تطابق تماماً حالة «المفتاح مطفأ». صار السبب يظهر على صفّ
التنزيل نفسه.

## v1.77.4

**إنستغرام 4.1.21 — ستوري الصورة ذات الموسيقى يُحفظ بصوته.** مسمّيات الريلز لا يجيب عليها عنصر
الستوري، فكان الخيار لا يُعرض أصلاً والصورة تُحفظ صامتة؛ المقطع يُقرأ الآن من شجرة الموسيقى في
قاموس العنصر — الفرع مؤكَّد من مرجع من نفس الجيل، والورقة تُبحث لا تُسمّى، وصور الغلاف مرفوضة
بالشكل. وزرّ العين صار يُبلّغ في صفحة التشخيص هل وجد ما يتابع به بدل أن يكتبه في السجل وحده.

## v1.77.3

**إنستغرام 4.1.20 — تحميل الستوري: التقرير قال إن التحميل لم يبدأ أصلاً.** سطر «آخر تنزيل عومل
كـ» رجع فارغاً، فالبحث عن العنصر رجع لا شيء ولم يُسأل قاموس 4.1.19 قط. العنصر يُؤخذ الآن من
وسيطَي `-fullscreenSectionController:willDisplayStoryModel:` — دالّة مربوطة أصلاً، ووسيطها لا
يُعاد تسميته كما تُعاد تسمية أسماء الأصناف — بعد التحقّق من أنه عنصر فعلاً، والبحث في العروض
يسمّي في صفحة التشخيص كل صنف يغطّي المنتصف ويحمل «Story» في اسمه بدل أن يفشل صامتاً.

## v1.77.2

**إنستغرام 4.1.19 — زر الستوري يحمّل الفيديو لا صورته.** ستوري الفيديو يحمل `IGVideo` فارغاً
كمنشور الصورة، فكان التحميل يسقط إلى الغلاف؛ يُقرأ الآن من قاموس العنصر (`video_versions` /
`video_dash_manifest`) بعد فشل مسار الكائن. إضافي، لا يمسّ ما يُحمَّل اليوم.

## v1.77.1

**X 0.18.6 — «بلا سجل بحث» يخفي السجل فعلاً ويمنع تسجيله.** كان يُفرِغ قائمة الأب بينما الصف
المرئي يرسمه متحكّمٌ ابن (`TTSRecentSearchTypeaheadViewController`)، والحسابات المبحوث عنها لم
تكن تُمَسّ أصلاً. ومعها حُجبت الكتابة في `TTSRecentSearchesDatastore`، وهو ما يجيب النصف الثاني من
البلاغ. كل صنف ومُحدِّد مقروء من ثنائي X عبر `tools/objc-classes.py`.
