...
**«بلا سجل بحث» كان يُفرِغ قائمةً لا أحد يراها.** بلاغ من جهاز: السجل يظهر ويُسجَّل.

الخطّاف كان على `TTSSearchTypeaheadViewController -setRecentQueries:` — وهي موجودة وتُستدعى
فعلاً، والعدّاد كان يتحرّك ويقول إن القوائم حُجبت. لكن **الصف الذي تراه العين يرسمه متحكّم آخر**:
`TTSRecentSearchTypeaheadViewController`، ابنٌ لذلك المتحكّم، له مُهيّئه و`results` خاصّته. فكنا
نفرّغ الأب ونترك الابن يرسم — عدّاد يتحرّك على مسارٍ ليس هو المسار.

مقيس من ثنائي X نفسه (12.20، `XAppLibraries.framework`) عبر `tools/objc-classes.py`، لا من
التخمين: الصنف يُصرّح `-setRecentQueries:` كما ظننّا، **ويُصرّح معها `recentUsers` و`recentUserIDs`
و`recentSearchViewController`** — فالسجل ليس ما كُتب فقط، بل الحسابات التي بُحث عنها كذلك، وثلاثة
مداخل لسطحٍ واحد أُفرِغ منها واحد.

**والنصف الثاني من البلاغ — «ويتسجّل» — لا يجيبه أي خطّاف عرض.** التسجيل في
`TTSRecentSearchesDatastore`، وهي تسمّي ما تفعله في مُحدِّدَيها: `-storeRecentSearchQuery:`
و`-storeRecentSearchUserID:`. الكتابة تُحجب الآن، وهي الطبقة الصادقة — لا يُسلَّم لأحدٍ سطرٌ نصف
مبنيّ ليمحوه، وهو الفرق الذي كلّف أداة الووتش انهياراً.

**و`-storeRecentSearchUserID:` تأخذ `q` (long long) لا كائناً** — مقروءة من ترميزها
`v24@0:8q16` لا من اسمها، وهي نفس الغلطة التي أسقطت تيك توك مرّتين.

ما لا تفعله: **لا تمحو المخزَّن أصلاً.** الصنف يعرض `clearRecentSearches` واستدعاؤها يحذف بيانات
شخصٍ لأنه شغّل مفتاحاً؛ خطّافات العرض تغطّي ما هو موجود، والحذف قرار صاحب الجهاز.

والتقرير صار ثلاثة أسطر بدل واحد — الاستعلامات والحسابات، والقوائم المرسومة، والكتابات المرفوضة —
لأن رقماً واحداً يغطّي ثلاثة أسطح هو ما سمح للميزة أن تعلن نجاحها بينما الصف الذي وُجدت لإخفائه
أمام العين.
